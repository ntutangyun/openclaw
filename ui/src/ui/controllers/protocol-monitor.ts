...
export type LatencyCacheSnapshot = {
  ...
  /**
   * Optional: controller-level mutable stores that the live UI builds up at
   * trace-ingest time. The export viewer parses a frozen snapshot and never
   * runs `recordMessageFromTrace` / `recordLlmCallFromTrace` /
   * `recordAgentLlmEventFromTrace` itself, so without these the Messages
   * section, Agent|Model section, and per-message Throughput section all
   * render empty. Optional so older exports (without these fields) still load
   * — they'll just have empty Messages/Agent|Model panels, same as today.
   */
  controllerStores?: ControllerStoresSnapshot;
};

/**
 * Snapshot of every module-level mutable store the Protocol Monitor view
 * reads from outside `netState`. Each field corresponds to a const declared
 * elsewhere in this file; the rehydrator restores them in place. Shapes are
 * inlined (rather than referencing the per-store local types) so the export
 * format is self-documenting without depending on declaration order in this
 * file.
 */
export type ControllerStoresSnapshot = {
  /** messageStore: per-direction MessageEntry buffers. */
  messages: {
    opGw: { forward: SerializedMessageEntry[]; reverse: SerializedMessageEntry[] };
    nodeGw: { forward: SerializedMessageEntry[]; reverse: SerializedMessageEntry[] };
  };
  /** llmCallStore (Map serialized as entries) + lastCallByRunId. */
  llmCalls: {
    entries: [string, LlmCall][];
    lastCallByRunId: [string, string][];
  };
  /** agentLlmEventStore — every recorded agent↔llm event bar. */
  agentLlmEvents: SerializedAgentLlmEvent[];
  /** pingSamples: per-source per-direction ping latency. */
  ping: {
    operator: { forward: SerializedPingSample[]; reverse: SerializedPingSample[] };
    node: { forward: SerializedPingSample[]; reverse: SerializedPingSample[] };
  };
  /** rxSamples: peer-reported gw→peer per-frame latency + size. */
  rxSamples: {
...
export function snapshotLatencyCaches(): LatencyCacheSnapshot {
  return {
    ...
    controllerStores: snapshotControllerStores(),
  };
}
...
export function rehydrateLatencyCaches(snapshot: LatencyCacheSnapshot) {
  ...
  if (snapshot.controllerStores) {
    rehydrateControllerStores(snapshot.controllerStores);
  }
}

/**
 * Snapshot every module-level store the Messages section, Throughput section,
 * Agent|Model section, and Latency section depend on. These are populated at
 * trace-ingest time on the live UI but are NOT regenerated by
 * `computeNetworkStats` (which only reads them) — without this snapshot, the
 * export viewer renders empty panels everywhere except the timeline.
 */
function snapshotControllerStores(): ControllerStoresSnapshot {
  return {
    messages: {
      opGw: {
        forward: messageStore.opGw.forward.map((e) => ({ ...e })),
        reverse: messageStore.opGw.reverse.map((e) => ({ ...e })),
      },
      nodeGw: {
        forward: messageStore.nodeGw.forward.map((e) => ({ ...e })),
        reverse: messageStore.nodeGw.reverse.map((e) => ({ ...e })),
      },
    },
    llmCalls: {
      entries: [...llmCallStore.entries()].map(([k, v]) => [k, { ...v }]),
      lastCallByRunId: [...lastCallByRunId.entries()],
    },
    agentLlmEvents: agentLlmEventStore.map((e) => ({ ...e })),
    ping: {
      operator: {
        forward: pingSamples.operator.forward.map((s) => ({ ...s })),
        reverse: pingSamples.operator.reverse.map((s) => ({ ...s })),
      },
      node: {
        forward: pingSamples.node.forward.map((s) => ({ ...s })),
        reverse: pingSamples.node.reverse.map((s) => ({ ...s })),
      },
    },
    rxSamples: {
...
function clearControllerStores(): void {
...
function rehydrateControllerStores(snap: ControllerStoresSnapshot): void {
  for (const e of snap.messages.opGw.forward) {
    messageStore.opGw.forward.push({ ...e });
  }
  for (const e of snap.messages.opGw.reverse) {
    messageStore.opGw.reverse.push({ ...e });
  }
  for (const e of snap.messages.nodeGw.forward) {
    messageStore.nodeGw.forward.push({ ...e });
  }
  for (const e of snap.messages.nodeGw.reverse) {
    messageStore.nodeGw.reverse.push({ ...e });
  }
  for (const [k, v] of snap.llmCalls.entries) {
    llmCallStore.set(k, { ...v });
  }
  for (const [k, v] of snap.llmCalls.lastCallByRunId) {
    lastCallByRunId.set(k, v);
  }
  for (const e of snap.agentLlmEvents) {
    agentLlmEventStore.push({ ...e });
  }
  for (const s of snap.ping.operator.forward) {
    pingSamples.operator.forward.push({ ...s });
  }
  for (const s of snap.ping.operator.reverse) {
    pingSamples.operator.reverse.push({ ...s });
  }
  for (const s of snap.ping.node.forward) {
    pingSamples.node.forward.push({ ...s });
  }
  for (const s of snap.ping.node.reverse) {
    pingSamples.node.reverse.push({ ...s });
  }
  for (const s of snap.rxSamples.operator) {
    rxSamplesOperator.push({ ...s });
  }
