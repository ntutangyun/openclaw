...
import { PING_INTERVAL_MS } from "./clock-sync.js";
...
export async function startGatewayServer(
  ...
  try {
    ...
    if (pingSampleStore) {
      ...
      // Ping each connected peer on PING_INTERVAL_MS (gateway → peer
      // direction). The peer ACKs via `ping.gw-to-peer.ack`; the handler
      // computes RTT and records a reverse sample in the ping store, which
      // broadcasts to UIs.
    ...
    const attachedGatewayExtraHandlers: Record<
      string,
      (typeof pluginRegistry.gatewayHandlers)[string]
    > = {
      ...pluginRegistry.gatewayHandlers,
      ...extraHandlers,
      // Protocol monitor traces — local fork addition; survives plugin reload
      // because the keys aren't in attachedPluginGatewayHandlerKeys.
      ...(protocolTraceStore
        ? {
